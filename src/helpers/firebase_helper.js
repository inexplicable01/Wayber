...
class FirebaseAuthBackend {
  constructor(firebaseConfig) {
    ...
    this.firestore = firebase.firestore(); // Properly reference Firestore
      ...
      firebase
        ...
        .then((result) => {
          ...
          firebase
            ...
            .collection("users")
            .doc(uid)
            .set({ ...additionalInfo, email: email })
            .then(() => {
  ...
  submitClientProfile = (profileData) => {
    const clientProfileCollection = firebase
      .firestore()
    ...
    return clientProfileCollection
      .add(extendedProfileData)
      .then((docRef) => {
  ...
  addDocumentToSign = async (uid, email, docRef, emails) => {
    if (!uid) return;
    const signed = false;
    const xfdf = [];
    const signedBy = [];
    ...
    const signedTime = "";
    this.firestore // this added here else everything is same
  ...
  searchForDocumentToSign = async (email) => {
    const documentsRef = this.firestore.collection("documentsToSign");
 ...
 searchForDocumentsSigned = async email => {
    const documentsRef = this.firestore.collection('documentsToSign');
 ...
 updateDocumentToSign = async (docId, email, xfdfSigned) => {
    const documentRef = this.firestore.collection('documentsToSign').doc(docId);
    documentRef
      .get()
      .then(async doc => {
        if (doc.exists) {
